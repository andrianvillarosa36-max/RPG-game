package com.chuck.rpggame;

import android.graphics.Canvas;
import android.graphics.Color;
import android.graphics.Paint;
import android.graphics.RectF;

public class PauseMenu {
    private final RectF panelBounds;
    private final RectF resumeButton;
    private final RectF settingsButton;
    private final RectF quitButton;

    private final Paint overlayPaint = new Paint();
    private final Paint panelPaint = new Paint();
    private final Paint titlePaint = new Paint();
    private final Paint buttonPaint = new Paint();
    private final Paint buttonTextPaint = new Paint();

    public PauseMenu(float w, float h) {
        float pw = w * 0.4f;
        float ph = h * 0.55f;
        panelBounds = new RectF((w - pw) / 2f, (h - ph) / 2f, (w + pw) / 2f, (h + ph) / 2f);

        float bw = pw * 0.8f;
        float bh = ph * 0.16f;
        float cx = panelBounds.centerX();
        float startY = panelBounds.top + ph * 0.35f;
        float gap = bh * 1.3f;

        resumeButton = new RectF(cx - bw / 2f, startY, cx + bw / 2f, startY + bh);
        settingsButton = new RectF(cx - bw / 2f, startY + gap, cx + bw / 2f, startY + gap + bh);
        quitButton = new RectF(cx - bw / 2f, startY + gap * 2, cx + bw / 2f, startY + gap * 2 + bh);

        overlayPaint.setColor(Color.argb(150, 0, 0, 0));
        panelPaint.setColor(Color.argb(235, 30, 35, 30));

        titlePaint.setColor(Color.WHITE);
        titlePaint.setTextSize(bh * 0.55f);
        titlePaint.setAntiAlias(true);
        titlePaint.setFakeBoldText(true);
        titlePaint.setTextAlign(Paint.Align.CENTER);

        buttonPaint.setColor(Color.argb(200, 70, 110, 70));

        buttonTextPaint.setColor(Color.WHITE);
        buttonTextPaint.setTextSize(bh * 0.4f);
        buttonTextPaint.setAntiAlias(true);
        buttonTextPaint.setTextAlign(Paint.Align.CENTER);
    }

    public void draw(Canvas canvas, float screenW, float screenH) {
        canvas.drawRect(0, 0, screenW, screenH, overlayPaint);
        canvas.drawRoundRect(panelBounds, 20, 20, panelPaint);
        canvas.drawText("Paused", panelBounds.centerX(), panelBounds.top + panelBounds.height() * 0.18f, titlePaint);

        drawButton(canvas, resumeButton, "Resume");
        drawButton(canvas, settingsButton, "Settings");
        drawButton(canvas, quitButton, "Quit to Title");
    }

    private void drawButton(Canvas canvas, RectF bounds, String label) {
        canvas.drawRoundRect(bounds, 16, 16, buttonPaint);
        canvas.drawText(label, bounds.centerX(), bounds.centerY() + buttonTextPaint.getTextSize() * 0.35f, buttonTextPaint);
    }

    public boolean isResume(float x, float y) {
        return resumeButton.contains(x, y);
    }

    public boolean isSettings(float x, float y) {
        return settingsButton.contains(x, y);
    }

    public boolean isQuit(float x, float y) {
        return quitButton.contains(x, y);
    }
}
